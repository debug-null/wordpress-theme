...
function optionsframework_tabs() {
	...
	foreach ( $options as $value ) {
		...
		if ( $value['type'] == "heading" ) {
			...
			$menu .= '<li><a id="options-group-'.  $counter . '-tab" class="' . $class .'" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#options-group-'.  $counter ) . '"> <i class="uk-icon-' . esc_html( $value['icon'] ) . '"> </i> ' . esc_html( $value['name'] ) . '</a></li>';
		}
	}
